...
encode_field_map :: proc(field_info: Field_Info) -> (field: wire.Field, ok: bool) {
    ...
    for entry_idx := 0; entry_idx < entry_count; entry_idx += 1 {
        ...
        key_ptr := runtime.map_cell_index_dynamic(ks, map_type.map_info.ks, uintptr(entry_idx))
        value_ptr := runtime.map_cell_index_dynamic(vs, map_type.map_info.vs, uintptr(entry_idx))

        key_field_info.data = Field_Data_Scalar(key_ptr)
        value_field_info.data = Field_Data_Scalar(value_ptr)

        entry_wire.fields[key_field_info.proto_id] = encode_field_scalar(key_field_info) or_return
...
@(private = "file")
encode_field_value :: proc(field: any, type: builtins.Type) -> (wire_value: wire.Value, ok: bool) {
    switch type {
    // VARINT-backing
    case .t_int32:
    ...
    case .t_enum:
        wire_value = builtins.encode_enum((transmute(^builtins.Enum_Wire_Type)field.data)^)
    case .t_sint32:
